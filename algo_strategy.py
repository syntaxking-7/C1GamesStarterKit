...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def wall_path_defense(self, game_state, target_location):
        """Defense placement based on attack location.
        
        FLUSHING BEHAVIOR: Each round independently analyzes the current attack.
        If we can't place all 3 hardcoded turrets due to low SP, we place what
        we can and discard the rest. Next round will freshly analyze the new
        attack location and determine new placements from scratch.
        
        Priority:
        1. If attack matches hardcoded pattern -> place those specific turrets only
        2. If no hardcoded match -> use generic defense placement
        """
        ...
        if hardcoded:
            # Hardcoded pattern matched - place ONLY these turrets
            # Any turrets we can't afford are discarded (not saved for next round)
            for loc in hardcoded:
                if game_state.get_resource(SP) < sp_cost:
                    # Out of SP - remaining positions are flushed/discarded
                    return
                if not game_state.contains_stationary_unit(loc):
                    game_state.attempt_spawn(TURRET, loc)
            # All hardcoded turrets placed (or attempted), done for this round
            return
        
        # No hardcoded pattern matched - use generic defense placement
